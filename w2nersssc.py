'''
    ...
    两个不同的学习率
    技巧使用： 1. 两个三对角添加新特征向量
    后缀c: 特征向量维度为实体标签类别数（7）
    后缀无c: 特征向量维度为bert的输出向量维度（768）
'''
import os
...
parser.add_argument('--check', type=str, default='model/w2nersssc', help='The path where model saved')
...
class W2NER(keras.layers.Layer):
    ...
    def call(self, inputs, **kwargs):
        ...
        # B*N*N*7
        logits = tri(logits, self.tri, N, params.label_num)
...
class USER:
    ...
    def plothistory(self):
        ...
        plt.tight_layout()
        plt.savefig("w2nersssc_PRF.jpg", dpi=500, bbox_inches="tight")
